...
public sealed class CustomBearerAuthenticationConcern : IConcern
{
    ...
    internal CustomBearerAuthenticationConcern(
        IHandler content,
    ...
    public async ValueTask<IResponse?> HandleAsync(IRequest request)
    {
        ...
        {
            if (ValidationOptions.KeyResolver != null)
            {
                var dummyToken = tokenHandler.ReadJwtToken(tokenString);
                _issuerKeys = await ValidationOptions.KeyResolver.Invoke(dummyToken);
            }
            else
            {
                _issuerKeys = await FetchSigningKeysFromWellKnown(issuer);
            }
        }
        ...
        if (issuer == null)
        {
            validationParameters.SignatureValidator = (token, p) =>
    ...
    private static async Task<ICollection<SecurityKey>> FetchSigningKeysFromWellKnown(string issuer)
    {
        try
        {
            ...
            var config =
                JsonSerializer.Deserialize<OpenIdConfiguration>(configResponse)
                ?? throw new InvalidOperationException(
                    $"Unable to discover configuration via '{configUrl}'"
                );
        ...
        catch (Exception ex)
        {
            throw new ProviderException(
                ...
                "Unable to fetch signing issuer signing keys",
                ex
            );
        }
    }
