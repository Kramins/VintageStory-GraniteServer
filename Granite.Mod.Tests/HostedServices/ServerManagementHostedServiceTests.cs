...
/// <summary>
/// Tests for ServerManagementHostedService.
...
/// </summary>
public class ServerManagementHostedServiceTests : IDisposable
{
    ...
    private readonly ServerManagementHostedService _service;

    public ServerManagementHostedServiceTests()
    {
        ...
        _service = new ServerManagementHostedService(
            _mockApi,
            _commandService,
            _messageBus,
            _config,
    ...
    {
        // Arrange & Act
        var act = () => new ServerManagementHostedService(
            null!,
            _commandService,
            _messageBus,
            _config,
    ...
    [Fact]
    public async Task SyncServerConfigCommand_IsSubscribedCorrectly()
    {
        // Arrange
        await _service.StartAsync(CancellationToken.None);
        var commandReceived = false;

        var command = new SyncServerConfigCommand
        {
            OriginServerId = _config.ServerId,
            Data = new SyncServerConfigCommandData()
        };

        _messageBus.GetObservable()
            .Where(msg => msg is SyncServerConfigCommand)
        ...
        // Assert
        commandReceived.Should().BeTrue("SyncServerConfigCommand should be received");
    }

    [Fact]
    public async Task UpdateServerConfigCommand_IsSubscribedCorrectly()
    {
        // Arrange
        await _service.StartAsync(CancellationToken.None);
        var commandReceived = false;

        var command = new UpdateServerConfigCommand
        {
            OriginServerId = _config.ServerId,
            Data = new UpdateServerConfigCommandData
            {
        ...
        _messageBus.GetObservable()
            .Where(msg => msg is UpdateServerConfigCommand)
            .Subscribe(_ => commandReceived = true);
        ...
        // Assert
        commandReceived.Should().BeTrue("UpdateServerConfigCommand should be received");
    }
}
