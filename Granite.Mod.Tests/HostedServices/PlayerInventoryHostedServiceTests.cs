using System.Reactive.Linq;
using FluentAssertions;
using GraniteServer.HostedServices;
using GraniteServer.Messaging.Commands;
using GraniteServer.Mod;
using GraniteServer.Services;
using NSubstitute;
using Vintagestory.API.Common;
using Vintagestory.API.Server;

namespace Granite.Mod.Tests.HostedServices;

/// <summary>
/// Tests for PlayerInventoryHostedService.
/// Note: Full integration testing requires a running VintageStory server instance.
/// These tests focus on service lifecycle and basic subscription validation.
/// </summary>
public class PlayerInventoryHostedServiceTests : IDisposable
{
    private readonly ICoreServerAPI _mockApi;
    private readonly ClientMessageBusService _messageBus;
    private readonly GraniteModConfig _config;
    private readonly ILogger _mockLogger;
    private readonly PlayerInventoryHostedService _service;

    public PlayerInventoryHostedServiceTests()
    {
        _mockApi = Substitute.For<ICoreServerAPI>();
        _config = new GraniteModConfig { ServerId = Guid.NewGuid() };
        _mockLogger = Substitute.For<ILogger>();
        _messageBus = new ClientMessageBusService(_mockLogger, _config);

        _service = new PlayerInventoryHostedService(
            _mockApi,
            _messageBus,
            _config,
            _mockLogger
        );
    }

    public void Dispose()
    {
        _service.Dispose();
    }

    [Fact]
    public async Task StartAsync_InitializesSuccessfully()
    {
        // Act
        await _service.StartAsync(CancellationToken.None);

        // Assert - No exception thrown means success
        _mockLogger.Received().Notification(Arg.Is<string>(s => s.Contains("started")));
    }

    [Fact]
    public async Task StopAsync_DisposesSubscriptionsSuccessfully()
    {
        // Arrange
        await _service.StartAsync(CancellationToken.None);

        // Act
        await _service.StopAsync(CancellationToken.None);

        // Assert - No exception thrown means success
        _mockLogger.Received().Notification(Arg.Is<string>(s => s.Contains("stopped")));
    }

    [Fact]
    public void Constructor_WithNullApi_ThrowsArgumentNullException()
    {
        // Arrange & Act
        var act = () => new PlayerInventoryHostedService(
            null!,
            _messageBus,
            _config,
            _mockLogger
        );
        
        // Assert
        act.Should().Throw<ArgumentNullException>().WithParameterName("api");
    }

    [Fact]
    public void Constructor_WithNullMessageBus_ThrowsArgumentNullException()
    {
        // Arrange & Act
        var act = () => new PlayerInventoryHostedService(
            _mockApi,
            null!,
            _config,
            _mockLogger
        );
        
        // Assert
        act.Should().Throw<ArgumentNullException>().WithParameterName("messageBus");
    }

    [Fact]
    public void Dispose_CalledMultipleTimes_DoesNotThrow()
    {
        // Arrange & Act
        _service.Dispose();
        var act = () => _service.Dispose();

        // Assert
        act.Should().NotThrow();
    }

    [Fact]
    public async Task QueryPlayerInventoryCommand_IsSubscribedCorrectly()
    {
        // Arrange
        await _service.StartAsync(CancellationToken.None);
        var commandReceived = false;

        var command = new QueryPlayerInventoryCommand
        {
            OriginServerId = _config.ServerId,
            Data = new QueryPlayerInventoryCommandData
            {
                PlayerId = "test-player-id"
            }
        };

        _messageBus.GetObservable()
            .Where(msg => msg is QueryPlayerInventoryCommand)
            .Subscribe(_ => commandReceived = true);

        // Act
        _messageBus.Publish(command);
        await Task.Delay(50);

        // Assert
        commandReceived.Should().BeTrue("QueryPlayerInventoryCommand should be received");
    }

    [Fact]
    public async Task UpdateInventorySlotCommand_IsSubscribedCorrectly()
    {
        // Arrange
        await _service.StartAsync(CancellationToken.None);
        var commandReceived = false;

        var command = new UpdateInventorySlotCommand
        {
            OriginServerId = _config.ServerId,
            Data = new UpdateInventorySlotCommandData
            {
                PlayerId = "test-player-id",
                InventoryName = "backpack",
                SlotIndex = 0
            }
        };

        _messageBus.GetObservable()
            .Where(msg => msg is UpdateInventorySlotCommand)
            .Subscribe(_ => commandReceived = true);

        // Act
        _messageBus.Publish(command);
        await Task.Delay(50);

        // Assert
        commandReceived.Should().BeTrue("UpdateInventorySlotCommand should be received");
    }

    [Fact]
    public async Task RemoveInventorySlotCommand_IsSubscribedCorrectly()
    {
        // Arrange
        await _service.StartAsync(CancellationToken.None);
        var commandReceived = false;

        var command = new RemoveInventorySlotCommand
        {
            OriginServerId = _config.ServerId,
            Data = new RemoveInventorySlotCommandData
            {
                PlayerId = "test-player-id",
                InventoryName = "backpack",
                SlotIndex = 0
            }
        };

        _messageBus.GetObservable()
            .Where(msg => msg is RemoveInventorySlotCommand)
            .Subscribe(_ => commandReceived = true);

        // Act
        _messageBus.Publish(command);
        await Task.Delay(50);

        // Assert
        commandReceived.Should().BeTrue("RemoveInventorySlotCommand should be received");
    }
}
